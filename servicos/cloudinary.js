...
const uploadImg = async function(img_path, id){
    ...
    try{
        let request = await fetch(process.env.IMGURL + "upload", {
            method: "POST",
            body: formData
        })
...
const deleteImg = async function( id){
    ...
    let assinatura = `public_id=${public_id}&timestamp=${timestamp}${process.env.IMGSEC}`
    ...
    try{
        let request = await fetch(process.env.IMGURL + "destroy", {
            method: "POST",
            body: formData
        })
